...
public static partial class Server
{
    ...
    /// </summary>
    private static Dictionary<string,string> AutoCertificateTokens = new();
    ...
    {
        if (Config.AutoCertificate.Email == null) throw new Exception("The auto certificate email is set to null.");
        ...
        try
        {
            ...
            if (testResponse != testToken) throw new Exception("Invalid response, aborting.");
        }
        catch
        {
            AutoCertificateTokens.Remove(testUrl);
        ...
        else
        {
            ...
            IAccountContext account = await acme.NewAccount(Config.AutoCertificate.Email, true);
            File.WriteAllText("../Certificates/Auto/account.pem", acme.AccountKey.ToPem());
        }

        //order+authenticate
        IOrderContext order = await acme.NewOrder(new[] {domain});
        ...
        while(true)
        {
            try
            {
                ...
                CertificateChain certificate = await order.Generate(new CsrInfo
                {
                    CommonName = domain
                }, privateKey);
    ...
    private async static Task CheckAutoCertificates()
    {
        ...
        List<string> coveredDomains = new();
        ...
        foreach (string path in Directory.GetFiles("../Certificates/Auto", "*.pfx", SearchOption.TopDirectoryOnly))
        {
            ...
            if (!domains.Contains(domain)) File.Delete(path);
        ...
        {
            if (coveredDomains.Contains(domain)) continue;
