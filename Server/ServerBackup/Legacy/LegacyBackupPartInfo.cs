...
/// </summary>
public class LegacyBackupPartInfo
{
    /// <summary>
    ...
    /// </summary>
    public readonly LegacyBackupTree LastKnownState;

    /// <summary>
    /// The directory tree for the changes since the previous backup.
    /// </summary>
    public readonly LegacyBackupTree Tree;
    ...
    /// </summary>
    public LegacyBackupPartInfo(string partName, string backupId, ReadOnlyCollection<string> backupBasedOnIds)
    {
    ...
    private static void LoadRecursive(StreamReader reader, LegacyBackupTree tree)
    {
        int read;
    ...
    public void BackupDirectory(string path)
    {
        ...
        //known tree
        LegacyBackupTree? knownTree = LastKnownState;
        ...
        //current tree
        LegacyBackupTree? lastExistingTree = null;
        LegacyBackupTree? firstMissingTree = null;
        string? missingTreeB64 = null;
        LegacyBackupTree currentTree = Tree;
        foreach (var componentB64 in componentsB64)
    ...
    private bool BackupDirectory(DirectoryInfo dir, LegacyBackupTree? knownTree, LegacyBackupTree currentTree, string backupToDir)
    {
        ...
        foreach (var d in dir.GetDirectories("*", SearchOption.TopDirectoryOnly))
        {
            ...
            //known tree
            if (knownTree == null || !knownTree.Directories.TryGetValue(b64, out LegacyBackupTree? k))
                k = null;

            //current tree
            bool potential;
            if (currentTree.Directories.TryGetValue(b64, out LegacyBackupTree? c) && c != null)
                potential = false;
            else
            {
                potential = true;
    ...
    private static void FindDeleted(LegacyBackupTree knownTree, LegacyBackupTree currentTree, string path)
    {
        //directories
        foreach (var dKV in knownTree.Directories)
        {
            ...
            if (Directory.Exists($"{path}{name}"))
            {
                //current
                bool potential;
                if (currentTree.Directories.TryGetValue(dKV.Key, out LegacyBackupTree? c) && c != null)
                    potential = false;
                else
                {
                    potential = true;
