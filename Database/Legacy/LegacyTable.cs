...
public class LegacyTable<T> : ILegacyTable, IEnumerable<KeyValuePair<string,T>> where T : ILegacyTableValue
{
    /// <summary>
    ...
    public static LegacyTable<T> Import(string name, bool skipBroken = false)
    {
        if (LegacyTables.Dictionary.TryGetValue(name, out ILegacyTable? table))
            return (LegacyTable<T>)table;
        if (!name.All(LegacyTables.KeyChars.Contains))
            throw new Exception($"This name contains characters that are not part of Tables.KeyChars ({LegacyTables.KeyChars}).");
        if (!Directory.Exists("../Database/" + name))
            return Create(name);
    ...
    {
        Dictionary<string, LegacyTableEntry<T>> data = [];
        ...
        foreach (FileInfo file in new DirectoryInfo("../Database/" + Name).EnumerateFiles("*.json"))
        {
            ...
            try
            {
                ...
                data[key] = new LegacyTableEntry<T>(Name, key, value, json);
    ...
    public LegacyTable<NewType> Transform<NewType>(Func<T,NewType> function) where NewType : ILegacyTableValue
    {
        LegacyTable<NewType> result = new(Name);
    ...
    {
        if (!newName.All(LegacyTables.KeyChars.Contains))
            throw new Exception($"This name contains characters that are not part of Tables.KeyChars ({LegacyTables.KeyChars}).");

        Directory.Move("../Database/"+Name, "../Database/"+ newName);
        LegacyTables.Dictionary[newName] = this;
        LegacyTables.Dictionary.Remove(Name);
        Name = newName;
        foreach (LegacyTableEntry<T> entry in Data.Values)
            entry.Table = newName;
    }
    ...
    {
        if (!newKey.All(LegacyTables.KeyChars.Contains))
            throw new Exception($"This key contains characters that are not part of Tables.KeyChars ({LegacyTables.KeyChars}).");
    ...
    public virtual T this[string key]
    {
        ...
        set
        {
            if (!key.All(LegacyTables.KeyChars.Contains))
                throw new Exception($"This key contains characters that are not part of Tables.KeyChars ({LegacyTables.KeyChars}).");
    ...
    /// </summary>
    protected virtual IEnumerable<string> EnumerateOtherFiles(LegacyTableEntry<T> entry)
        => [];

    /// <summary>
    /// Enumerates other directories that are associated with a given entry while it is locked (for backups).
    /// </summary>
    protected virtual IEnumerable<string> EnumerateOtherDirectories(LegacyTableEntry<T> entry)
        => [];
    ...
    public void Backup(string id, ReadOnlyCollection<string> basedOnIds)
    {
        ...
        try
        {
            LegacyBackupPartInfo backupPart = new(Name, id, basedOnIds);
    ...
    public void Restore(ReadOnlyCollection<string> ids)
    {
        ...
        //restore
        var restorePart = new LegacyRestorePartInfo(Name, ids);
        restorePart.Restore();
